...
import { UserReferralsView } from './UserReferralsView';

export const MarketingCampaignsTab = () => {
  const { token } = useAuth();

  // Active Section Tab: 'user_referrals' | 'campaigns'
  const [activeSection, setActiveSection] = useState('user_referrals');

  // User Referral (Earn & Refer) Data State
  const [userReferralSummary, setUserReferralSummary] = useState(null);
  const [userReferralLinks, setUserReferralLinks] = useState([]);
  const [userRegistrations, setUserRegistrations] = useState([]);
  const [userActivity, setUserActivity] = useState([]);
  const [userReferralLoading, setUserReferralLoading] = useState(false);
  ...
  const fetchData = async (silent = false) => {
    try {
      ...
      // Also refresh live user referral program data
      fetchUserReferrals(silent);
    } catch (err) {
  ...
  const fetchUserReferrals = async (silent = false) => {
    try {
      if (!silent) setUserReferralLoading(true);
      const headers = { Authorization: `Bearer ${token}` };

      const [sumRes, linksRes, regsRes, actRes] = await Promise.all([
        fetch('/api/marketing/user-referrals/summary', { headers }).catch(() => null),
        fetch('/api/marketing/user-referrals/links?limit=300', { headers }).catch(() => null),
        fetch('/api/marketing/user-referrals/registrations?limit=100', { headers }).catch(() => null),
        fetch('/api/marketing/user-referrals/activity?limit=50', { headers }).catch(() => null),
      ]);

      if (sumRes && sumRes.ok) {
        setUserReferralSummary(await sumRes.json());
      }
      if (linksRes && linksRes.ok) {
        setUserReferralLinks(await linksRes.json());
      }
      if (regsRes && regsRes.ok) {
        setUserRegistrations(await regsRes.json());
      }
      if (actRes && actRes.ok) {
        setUserActivity(await actRes.json());
      }
    } catch (err) {
      console.error('Error fetching user referrals data:', err);
    } finally {
      if (!silent) setUserReferralLoading(false);
    }
  };
  ...
  const handleSimulateUserClick = async (code, deviceType = 'desktop') => {
    try {
      ...
      setCopyFeedback(`⚡ Simulated ${deviceType.toUpperCase()} click recorded for ${code}!`);
      setTimeout(() => setCopyFeedback(''), 3000);
      await fetchUserReferrals(true);
    } catch (err) {
      console.error('Error simulating user click:', err);
    }
  };
  ...
  return (
    <div className="marketing-tab-container" style={{ width: '100%', maxWidth: '100%', boxSizing: 'border-box', color: '#F8FAFC' }}>
      ...
      {activeSection === 'user_referrals' ? (
        <UserReferralsView
          summary={userReferralSummary}
          links={userReferralLinks}
