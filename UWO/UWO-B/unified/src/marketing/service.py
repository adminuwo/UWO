...
class MarketingService:
    ...
    def record_click(
        ...
        if not link or not link.get("is_active", True):
            # Check if this slug exists in UWO Referral ecosystem (ref_links in UWO-web database)
            try:
                from src.marketing.referral_service import get_uwo_db
                uwo_db = get_uwo_db()
                ref_link = uwo_db.ref_links.find_one({"code": slug})
                if ref_link:
                    now = datetime.now(timezone.utc)
                    ua_parsed = MarketingService.parse_user_agent(user_agent)

                    # Determine device type
                    raw_dev = ua_parsed.get("device", "desktop").lower()
                    raw_os = ua_parsed.get("os", "").lower()
                    if "ios" in raw_os or "iphone" in raw_os or "ipad" in raw_os:
                        device_type = "ios"
                    elif "android" in raw_os or "mobile" in raw_dev:
                        device_type = "android"
                    else:
                        device_type = "desktop"

                    # Check uniqueness
                    existing_click = uwo_db.ref_click_logs.find_one({
                        "referralLink": ref_link["_id"],
                        "ip": ip
                    })
                    is_unique = (existing_click is None)

                    # Insert ClickLog into ref_click_logs
                    uwo_db.ref_click_logs.insert_one({
                        "referralLink": ref_link["_id"],
                        "code": slug,
                        "user": ref_link.get("user"),
                        "userId": ref_link.get("userId"),
                        "product": ref_link.get("product"),
                        "deviceType": device_type,
                        "userAgent": user_agent[:250] if user_agent else "",
                        "ip": ip,
                    ...
                    # Increment clicks & uniqueClicks in ref_links
                    inc_update = {"clicks": 1}
                    if is_unique:
                        inc_update["uniqueClicks"] = 1
                    uwo_db.ref_links.update_one(
                        {"_id": ref_link["_id"]},
                        {"$inc": inc_update, "$set": {"updatedAt": now}}
                    )

                    # Lookup product for target URL
                    product = uwo_db.ref_products.find_one({"_id": ref_link.get("product")})
                    dest_url = "https://uwo24.com"
                    if product:
                        if device_type == "android" and product.get("androidUrl"):
                            dest_url = product["androidUrl"]
                            if "play.google.com" in dest_url:
                                sep = "&" if "?" in dest_url else "?"
                                dest_url = f"{dest_url}{sep}referrer=utm_source%3Dreferral%26ref%3D{slug}%26ref_by%3D{ref_link.get('userId', '')}"
                        elif device_type == "ios" and product.get("iosUrl"):
                            dest_url = product["iosUrl"]
                        else:
                            dest_url = product.get("webUrl") or product.get("androidUrl") or product.get("iosUrl") or "https://uwo24.com"

                    return dest_url
            except Exception as e:
                print(f"[MarketingService record_click ref_links notice] {e}")
    ...
    def record_install(
        ...
        # 6. Also update UWO Referral Ecosystem (ref_links & ref_download_logs in UWO-web)
        ref_attributed = False
        try:
            from src.marketing.referral_service import get_uwo_db
            uwo_db = get_uwo_db()
            ref_match = uwo_db.ref_links.find_one({"code": target_slug})
            if ref_match:
                ref_attributed = True
                uwo_db.ref_download_logs.insert_one({
                    "referralLink": ref_match["_id"],
                    "code": target_slug,
                    "user": ref_match.get("user"),
                    "userId": ref_match.get("userId"),
                    "product": ref_match.get("product"),
                    "platform": norm_platform,
                    "deviceId": device_id or f"dev_{uuid.uuid4().hex[:8]}",
                    "ip": ip,
                    "verified": True,
                    "attributionMethod": "ios_ip_fingerprint" if norm_platform == "ios" else "google_play_referrer",
                    "createdAt": now,
                    "updatedAt": now
                })
                uwo_db.ref_links.update_one(
            ...
            print(f"[MarketingService record_install ref_links notice] {e}")

        return {
            "success": True,
            "slug": target_slug or "unknown",
            "attributed": bool(link) or ref_attributed,
            "attribution_method": attribution_method,
            "product_id": resolved_product,
            "platform": norm_platform,
            "fingerprint": clean_fp or None,
