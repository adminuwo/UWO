...
const sendAdminNotification = async (data) => {
    ...
    const adminSubject = `New Lead: ${data.source || 'Contact Form'} Submission`;
    const adminHtml = `
...
app.post('/api/referrals', async (req, res) => {
    try {
        ...
        try {
            ...
            const { generateUserId, generatePassword } = require('./referral/middleware/auth');

            const cleanEmail = email.trim().toLowerCase();
            const cleanName = name.trim();

            let refUser = await RefUser.findOne({ email: cleanEmail });
            if (refUser) {
                userDashboardPassword = generatePassword();
                const salt = await bcrypt.genSalt(10);
                refUser.password = await bcrypt.hash(userDashboardPassword, salt);
                refUser.name = cleanName;
                await refUser.save();
                userDashboardUserId = refUser.userId;
                console.log(`ℹ️ Existing referral user password updated: User ID "${userDashboardUserId}" for ${cleanEmail}`);
            } else {
                ...
                userDashboardPassword = generatePassword();
                const salt = await bcrypt.genSalt(10);
                const hashedPassword = await bcrypt.hash(userDashboardPassword, salt);

                refUser = await RefUser.create({
                    userId: uId,
                    name: cleanName,
                    email: cleanEmail,
                ...
                userDashboardUserId = refUser.userId;
                console.log(`✅ Referral user account created: User ID "${userDashboardUserId}" for ${cleanEmail}`);
            }
        } catch (regErr) {
        ...
        res.status(201).json({
            ...
            credentials: {
                userId: userDashboardUserId,
                password: userDashboardPassword,
                email: email.trim().toLowerCase(),
                loginUrl: userDashboardLoginUrl
            }
        });
    } catch (err) {
